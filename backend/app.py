...
fertilizer_df = pd.read_csv('../disease_fertilizer.csv')
...
# --- MODIFIED & CORRECTED FERTILIZER FUNCTION ---
def get_fertilizer_recommendation(disease_name):
    ...
    if not row.empty:
        ...
        recommendations = []
        if pd.notna(fert_1) and fert_1.lower() != 'none':
            recommendations.append(fert_1)
        if pd.notna(fert_2) and fert_2.lower() != 'none':
            recommendations.append(fert_2)
            
        # Create separate strings for fertilizers and notes
        fertilizer_string = " or ".join(recommendations) if recommendations else "None"
        notes_string = notes if pd.notna(notes) else ""

        # Handle healthy plants specifically for clarity
    ...
    return {"fertilizers": "Unknown", "notes": "No recommendation available in the database for this condition."}
...
def predict_disease():
    ...
    try:
        ...
        if predicted_class_index >= len(class_names):
             raise IndexError("Prediction index is out of bounds.")

        predicted_class_name = class_names[predicted_class_index]
        confidence = float(np.max(predictions[0]))

        # The recommendation is now an object
