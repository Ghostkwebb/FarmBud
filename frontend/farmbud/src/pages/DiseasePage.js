...
import { Upload, Loader2, X, Microscope, Stethoscope, Leaf, CheckCircle2 } from 'lucide-react';
import {
  Accordion,
  AccordionContent,
  AccordionItem,
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="container py-8 flex justify-center items-center">
      <Card className="w-full max-w-4xl backdrop-blur-xl bg-card/60 border-border/20 grid grid-cols-1 md:grid-cols-2 gap-8 p-6">
        ...
        <div className="flex flex-col items-center justify-center gap-4">
          <div 
            className="w-full h-80 border-2 border-dashed border-muted-foreground rounded-lg flex items-center justify-center text-center p-4 relative cursor-pointer"
            onClick={() => fileInputRef.current.click()}
          >
            {preview ? (
              <>
                <img src={preview} alt="Preview" className="max-h-full max-w-full object-contain rounded-md" />
                <Button variant="destructive" size="icon" className="absolute top-2 right-2 h-8 w-8" onClick={(e) => { e.stopPropagation(); clearSelection(); }}>
                  <X className="h-4 w-4" />
                </Button>
              </>
            ) : (
        ...
        {/* --- MODIFIED: Right Side - Redesigned Results --- */}
          ...
          <AnimatePresence>
            ...
            {result ? (
              ...
              >
                ...
                <div>
                  <h3 className="text-sm font-semibold text-muted-foreground flex items-center mb-2">
                    <Stethoscope className="mr-2 h-4 w-4"/> CONDITION
                  </h3>
                ...
                <Accordion type="single" collapsible className="w-full" defaultValue="item-1">
                  <AccordionItem value="item-1">
                    ...
                    <AccordionContent className="text-base text-muted-foreground space-y-4">
                      {/* --- NEW: Display fertilizers and notes separately --- */}
                      <div>
                      ...
                      {result.recommendation.notes && (
                        <div>
                          ...
                          <p>{result.recommendation.notes}</p>
                        </div>
                      )}
                    </AccordionContent>
                  </AccordionItem>
              ...
              <div className="text-center text-muted-foreground h-full flex flex-col justify-center items-center">
                ...
                <p>Upload an image of a plant leaf to detect diseases and get fertilizer recommendations.</p>
              </div>
            )}
          </AnimatePresence>
        </div>
