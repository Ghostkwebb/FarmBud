...
import { Upload, Loader2, X, Microscope, Stethoscope, CheckCircle2, Cpu, Leaf } from 'lucide-react';
...
const HowItWorksStep = ({ icon, title, children }) => (
    <div className="flex items-start gap-4">
        ...
        <div>
            <TextToSpeech><h4 className="text-lg font-semibold text-foreground">{title}</h4></TextToSpeech>
            <TextToSpeech><p className="text-muted-foreground">{children}</p></TextToSpeech>
        </div>
    </div>
);
      ...
      <Card className="w-full max-w-4xl backdrop-blur-xl bg-card/60 border-border/20 grid grid-cols-1 md:grid-cols-2 gap-8 p-6">
        <div className="flex flex-col items-center justify-center gap-4">
          <div className="w-full h-80 border-2 border-dashed border-muted-foreground rounded-lg flex items-center justify-center text-center p-4 relative cursor-pointer" onClick={() => fileInputRef.current.click()}>
            {preview ? (
              <><img src={preview} alt="Preview" className="max-h-full max-w-full object-contain rounded-md" /><Button variant="destructive" size="icon" className="absolute top-2 right-2 h-8 w-8" onClick={(e) => { e.stopPropagation(); clearSelection(); }}><X className="h-4 w-4" /></Button></>
            ) : (
              <div className="text-muted-foreground"><Upload className="mx-auto h-12 w-12" /><p>Click to upload an image</p><p className="text-xs">PNG, JPG up to 10MB</p></div>
            )}
          </div>
          ...
          <AnimatePresence mode="wait">
            ...
            {result ? (
              <motion.div key="result" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                <TextToSpeech><h2 className="text-3xl font-bold">Diagnosis Result</h2></TextToSpeech>
                <div>
                  ...
                  <div className={`flex items-center gap-3 p-3 rounded-md ${result.disease.includes('healthy') ? 'bg-green-500/20 text-green-300' : 'bg-destructive/20 text-destructive-foreground'}`}>
                    ...
                    <div className="flex-grow">
                      <TextToSpeech><p className="text-xl font-bold capitalize">{result.disease}</p></TextToSpeech>
                    ...
                    <AccordionItem value="item-1">
                      ...
                      <AccordionContent className="text-base text-muted-foreground space-y-4">
                        <div>
                          ...
                          <TextToSpeech><p>{result.recommendation.fertilizers}</p></TextToSpeech>
                        </div>
                        {result.recommendation.notes && (<div><h4 className="font-semibold text-foreground mb-1">Notes:</h4><TextToSpeech><p>{result.recommendation.notes}</p></TextToSpeech></div>)}
                      </AccordionContent>
                    </AccordionItem>
                  </Accordion>
                )}
              ...
              <motion.div key="guide" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
                 <TextToSpeech><h2 className="text-3xl font-bold">How It Works</h2></TextToSpeech>
                 <div className="space-y-6">
                    <HowItWorksStep icon={<Upload className="h-6 w-6"/>} title="1. Upload Your Image">Select a clear photo of the plant leaf you are concerned about.</HowItWorksStep>
                    <HowItWorksStep icon={<Cpu className="h-6 w-6"/>} title="2. AI-Powered Analysis">Our trained model scans the image for patterns and signs of common diseases.</HowItWorksStep>
                    <HowItWorksStep icon={<Stethoscope className="h-6 w-6"/>} title="3. Instant Diagnosis">Receive a confident diagnosis and actionable recommendations in seconds.</HowItWorksStep>
                 </div>
              </motion.div>
            )}
          </AnimatePresence>
