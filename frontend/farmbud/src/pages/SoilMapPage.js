...
export default function SoilMapPage() {
  ...
  const handleAnalyzeSoil = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error('Soil analysis failed. Please try another image.');
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="container py-8">
      <Card className="w-full backdrop-blur-xl bg-card/60 border-border/20">
        <CardHeader>
          <CardTitle className="text-3xl">Get Data for Prediction</CardTitle>
          <div className="flex items-center text-sm text-muted-foreground mt-2">
            <span className={`flex items-center ${step === 1 ? 'text-primary font-semibold' : ''}`}>
              <Leaf className="mr-2 h-4 w-4"/> Step 1: Analyze Soil
            </span>
            <ChevronsRight className="mx-2 h-5 w-5"/>
            <span className={`flex items-center ${step === 2 ? 'text-primary font-semibold' : ''}`}>
              <LocateFixed className="mr-2 h-4 w-4"/> Step 2: Get Weather
            </span>
          </div>
        </CardHeader>
        <CardContent>
            ...
            {step === 1 && (
              <motion.div key="step1" exit={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} initial={{ opacity: 0, x: -50 }}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  {/* --- MODIFIED: Right Side - India Soil Map --- */}
                  <div className="flex flex-col justify-center">
                    <h3 className="text-xl font-semibold text-foreground mb-4">Major Soil Types of India</h3>
            ...
            {step === 2 && (
              <motion.div key="step2" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }}>
                ...
                <div className="mt-6 flex justify-between items-center">
                    ...
                    <Button onClick={handleApplyToPrediction} disabled={!weatherData}>
...
// --- The LocationStep sub-component remains unchanged ---
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div>
            <div className="p-4 bg-secondary/80 rounded-lg mb-4">
                <h3 className="font-bold text-lg">Soil Analysis Complete</h3>
                <p>Type: <span className="font-semibold">{soilData.soil_type}</span> ({soilData.confidence})</p>
                <ul className="text-sm">
                    ...
                    <li>pH: {soilData.nutrients.ph}</li>
                </ul>
            </div>
